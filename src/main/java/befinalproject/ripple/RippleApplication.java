...
public class RippleApplication {
	...
	public CommandLineRunner demo(PostRepository postrepository, TopicRepository topicrepository, AppUserRepository userrepository) {
		return (args) -> {
			...
			postrepository.save(new Post("What should I make from leftover apples?", "SuperFoodie", LocalDate.parse("2025-01-01", formatter), LocalDate.parse("2026-01-01", formatter), 1, 10, 1, topicrepository.findByName("Cooking").get(0), "Too hungry to think. Help."));
			postrepository.save(new Post("Useful programming languages to learn", "TechFreak", LocalDate.parse("2025-02-02", formatter), LocalDate.parse("2026-02-02", formatter), 2, 20, 2, topicrepository.findByName("Technology").get(0), "What do employers look for nowadays?"));
			postrepository.save(new Post("My fitness journey 2024-2025", "GymGuru", LocalDate.parse("2025-03-03", formatter), LocalDate.parse("2026-03-03", formatter), 3, 30, 3, topicrepository.findByName("Fitness").get(0), "#Dead"));
